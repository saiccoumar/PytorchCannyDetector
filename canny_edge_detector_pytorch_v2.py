...
def gaussian_blur(images, kernel_size=3, sigma=0.0):
    print("Step 2")   
...
def double_threshold_hysteresis(image_tensor, low_thresh, high_thresh):
...
def canny_edge_detection(images, low_threshold=20, high_threshold=50, kernel_size=5, sigma=1.0):
    i = 0
    ...
    blurred_image = gaussian_blur(gray_images, kernel_size, sigma)
    print(blurred_image.shape)
    for i in range(len(images)):
        show(blurred_image[i])
    
    # Step 3: Gradient Calculation
    gradients, angles = sobel_filters(blurred_image)
    
    # Step 4: Non-Maximum Suppression
    non_max_suppressed = non_maximum_suppression(gradients, angles)
    print(non_max_suppressed.shape)
    for i in range(len(images)):
        show(non_max_suppressed[i])
  
    # Step 5: Double Thresholding and Edge Tracking by Hysteresis
    edges = double_threshold_hysteresis(non_max_suppressed, 0, 1)[0]
    print(edges.shape)
    for i in range(len(images)):
        show(edges[i])
...
image = cv2.imread('ascii.png')
image2 = cv2.imread('kitty.webp')
image3 = cv2.imread('kuromi.webp')
...
images = np.array([image2, image, image3])
